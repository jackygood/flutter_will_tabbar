...
class WillTabBar extends StatefulWidget {
  ...
  const WillTabBar(
      ...
      : assert(tabBarItemTitles.length < 6,
            'tabBarItemTitles must be less than 5');
...
class _WillTabBarState extends State<WillTabBar> {
  ...
  void onPostFrameCallback(Duration duration) {
    if (!_showSelf) {
      setState(() {
        ...
        _maxWidth = _maxWidth! -
            (widget.horizontalMargin * 2 / widget.tabBarItemTitles.length);
        _showSelf = true;
      });
    }
  }
  ...
  List<Widget> tabBars() =>
      List<Widget>.generate(widget.tabBarItemTitles.length, (int index) {
        return Expanded(
            flex: 1,
                  ...
                  children: <Widget>[
                    Text(
                      ...
                      style: index == widget.currentValue!()
                          ? TextStyle(
                              fontSize: widget.selectTextFontSize,
                              color: widget.selectTextColor)
                          : TextStyle(
                              fontSize: widget.normalTextFontSize,
  ...
  Alignment _getAlignment() {
    return Alignment(
        -1.0 +
            widget.currentValue!() / (widget.tabBarItemTitles.length - 1) * 2,
        1.0);
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (!_showSelf) {
      ...
      return Container(
        margin: EdgeInsets.fromLTRB(widget.horizontalMargin,
            widget.horizontalMargin, widget.horizontalMargin, 0),
        height: 50,
        decoration: BoxDecoration(
            color: Colors.white,
